...
description: "Ask Claude plain-English questions about your own CRM data — leads, analytics, pricing and load board posts."
---
...
- _"How often do customers ask about enclosed transport?"_
- _"Export every conversation from last month as JSONL"_
...
| `get-lead-communications` | Read chat messages and call transcripts for one lead. Supports filtering by channel (SMS, call, email, WhatsApp) and incremental fetching to get only new messages. |

### Conversations, in bulk

| Tool | Description |
| --- | --- |
| `get-conversation-stats` | How much history matches a set of filters **before** you export it — conversation and message counts, the period covered, a breakdown by channel, direction and sender, and a sampled size estimate. Call this first for any range you have not exported before. |
| `search-conversations` | Find individual messages across every lead — message bodies, email subjects **and** call transcripts — with each hit carrying its own lead context. Newest first. Use it for "how often do customers ask about X" or "find the message where Y happened". |
| `export-conversations` | Every lead's conversations in one call, chat messages and call transcripts together, filtered by lead, status, channel, sender, direction or date. Returned inline, or written to a downloadable file as JSON, JSONL or CSV. Paging is exact — no duplicates, no gaps, safe while new messages arrive. |

<Tip>
  Ask `get-conversation-stats` before `export-conversations` on any large range. It tells Claude how
  big the job is so it can page through it properly instead of truncating.
</Tip>
...
## The same thing inside the CRM

If you would rather not leave BeRocker, the built-in **AI Assistant** (the purple **Ai** button in
the top bar) uses the same tools and answers the same questions.

→ [AI Assistant](/using/ai-assistant)
