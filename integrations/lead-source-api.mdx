...
<Note>
  ...
  [Lead Source API with pickup windows](/integrations/pickup-windows-api). If you are a lead provider
...
<Tip>
  To offer pickers instead of free-text vehicle fields, read the year, make and model lists from the
  [Vehicle database API](/integrations/vehicle-database-api) — the names it returns are exactly what
  `vehicle_make` and `vehicle_model` expect, and each model comes with its `vehicle_type`.
</Tip>
